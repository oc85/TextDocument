...
# Reduced horizontal padding slightly to avoid border lines
VALUE_PAD_X = 0.04
VALUE_PAD_Y = 0.04
...
def normalise_element(text):
    ...
    # 1. Check direct short mappings (e.g. "a1" -> "Al")
    mapped = apply_mapping(cleaned, ELEMENT_MAPPING)
    if mapped is not None:
        return mapped, True

    # 2. Check full chemical names (e.g. "Aluminium" -> "Al")
    lower_cleaned = cleaned.lower()
    if lower_cleaned in FULL_CHEMICAL_NAMES:
        return FULL_CHEMICAL_NAMES[lower_cleaned], True

    # 3. Clean token check for symbols
    token = re.sub(r"[^A-Za-z]", "", cleaned)
    if not token:
    ...
    if len(token) == 1:
        ...
        # Return full text as fallback rather than failing
        return cleaned, False
...
def normalise_value(text):
    ...
    value = original.translate(str.maketrans({
        "O": "0", "o": "0",
        "I": "1", "l": "1", "|": "1",
        ",": ".",
    }))
    ...
    # FIX: Clean phantom trailing '1's produced by cell border lines (e.g. 20001 -> 2000)
...
# ============================================================
# GUI CLASS WITH COPY FUNCTIONALITY
# ============================================================
...
class OCRInspectionTool:
    ...
    def _build_gui(self):
        ...
        result_frame = ttk.Frame(self.main_pane, width=430)

        self.main_pane.add(image_frame, weight=5)
        self.main_pane.add(result_frame, weight=1)
        ...
        # Result Panel Header
        ttk.Label(result_frame, text="YOLO detections and OCR results", font=("Segoe UI", 10, "bold")).pack(anchor="w", pady=(2, 5))

        # Legend
        legend_frame = ttk.LabelFrame(result_frame, text="YOLO box colours", padding=5)
        legend_frame.pack(fill=tk.X, pady=(0, 7))

        self.colour_legend = tk.Text(legend_frame, height=6, wrap=tk.NONE, relief=tk.FLAT, borderwidth=0, background="#f7f7f7", cursor="arrow", font=("Segoe UI", 9))
        ...
        # Timing Info
        timing_frame = ttk.LabelFrame(result_frame, text="Processing time", padding=5)
        timing_frame.pack(fill=tk.X, pady=(0, 7))

        self.timing_text = tk.Text(timing_frame, height=7, wrap=tk.NONE, relief=tk.FLAT, borderwidth=0, background="#f7f7f7", cursor="arrow", font=("Consolas", 8))
        ...
        self.table = ttk.Treeview(table_container, columns=columns, show="headings", selectmode="extended")

        headings = {"no": "#", "class": "Class", "raw": "Raw OCR", "final": "Final", "det_conf": "YOLO", "ocr_conf": "OCR", "warning": "!"}
        widths = {"no": 35, "class": 105, "raw": 90, "final": 90, "det_conf": 50, "ocr_conf": 50, "warning": 30}

        for column in columns:
            self.table.heading(column, text=headings[column])
            self.table.column(column, width=widths[column], minwidth=30, anchor="w")

        table_y_scroll = ttk.Scrollbar(table_container, orient=tk.VERTICAL, command=self.table.yview)
        table_x_scroll = ttk.Scrollbar(table_container, orient=tk.HORIZONTAL, command=self.table.xview)
        ...
        # ====================================================
        # NEW FEATURE: COPY TABLE RESULTS TO CLIPBOARD
        # ====================================================
        self._setup_table_copy_feature()

        ttk.Label(result_frame, text="Wheel: zoom   |   Drag: move page   |   Right-click table: copy data", foreground="#666666", padding=(0, 5)).pack(anchor="w")

        self._set_navigation_state()

    # --------------------------------------------------------
    # COPY FUNCTIONALITY METHOD
    # --------------------------------------------------------
    def _setup_table_copy_feature(self):
        # Create Context Menu
        self.table_context_menu = tk.Menu(self.root, tearoff=0)
        self.table_context_menu.add_command(label="Copy Selected Row(s)", command=self.copy_selected_rows)
        self.table_context_menu.add_command(label="Copy Entire Table", command=self.copy_entire_table)

        # Bindings for Right Click and Short-keys
        self.table.bind("<Button-3>", self._show_table_context_menu)
        self.table.bind("<Control-c>", lambda e: self.copy_selected_rows())
        self.table.bind("<Control-a>", lambda e: self.select_all_rows())
    ...
    def copy_selected_rows(self):
        ...
        lines = []
        # Header row
        headers = ["#", "Class", "Raw OCR", "Final", "YOLO", "OCR", "!"]
        lines.append("\t".join(headers))
        
        for item in selected_items:
            vals = [str(v) for v in self.table.item(item, "values")]
            lines.append("\t".join(vals))
            
        clipboard_text = "\n".join(lines)
        self.root.clipboard_clear()
        self.root.clipboard_append(clipboard_text)
        self.set_status("Selected table rows copied to clipboard.", "#137333")
    ...
    def mouse_zoom(self, event):
        ...
        image_x, image_y = canvas_x / old_zoom, canvas_y / old_zoom
        self.zoom = new_zoom
        self.show_image(image)

        new_canvas_x, new_canvas_y = image_x * new_zoom, image_y * new_zoom
        bbox = self.canvas.bbox("all")

        if bbox:
            total_width = max(1, bbox[2] - bbox[0])
            total_height = max(1, bbox[3] - bbox[1])
            desired_left = new_canvas_x - event.x
            desired_top = new_canvas_y - event.y
            self.canvas.xview_moveto(max(0.0, min(1.0, desired_left / total_width)))
            self.canvas.yview_moveto(max(0.0, min(1.0, desired_top / total_height)))
